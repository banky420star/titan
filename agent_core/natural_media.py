"""Natural language media router — routes plain-English media commands."""
...
GIF_PREFIXES = [
    "create gif", "create a gif", "create short gif", "create a short gif",
    "make gif", "make a gif", "make short gif", "make a short gif",
    "generate gif", "generate a gif",
    "animate", "create animation", "make animation",
]
...
def route_natural_media(command):
    ...
    if image_prompt is not None:
        if not image_prompt:
            return {"handled": True, "text": "Usage: create image <what you want to see>"}
        result = create_image(image_prompt)
        return {"handled": True, "text": json.dumps(result, indent=2)}
    ...
    if gif_prompt is not None:
        if not gif_prompt:
            return {"handled": True, "text": "Usage: create gif <what should move>"}
    ...
    if low in ["show videos", "list videos", "my videos", "open videos"]:
        return {"handled": True, "text": json.dumps(list_videos(), indent=2)}

    if low in ["show images", "list images", "my images", "open images"]:
        return {"handled": True, "text": json.dumps(list_images(), indent=2)}
