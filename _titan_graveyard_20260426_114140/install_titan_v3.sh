...
mkdir -p assets skills subagents rag/docs rag/chroma workspace logs

cat > assets/titan_pixel_diva.svg <<'SVG'
<svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
  <rect width="512" height="512" rx="96" fill="#14121f"/>
  <rect x="56" y="56" width="400" height="400" rx="52" fill="#211a33"/>
  <g shape-rendering="crispEdges">
    <rect x="160" y="88" width="192" height="32" fill="#ff9f43"/>
    <rect x="128" y="120" width="256" height="32" fill="#ff9f43"/>
    <rect x="112" y="152" width="288" height="64" fill="#ff7a18"/>
    <rect x="144" y="216" width="224" height="32" fill="#ffd1a8"/>
    <rect x="160" y="248" width="192" height="32" fill="#ffd1a8"/>
    <rect x="176" y="280" width="160" height="64" fill="#111827"/>
    <rect x="144" y="344" width="64" height="48" fill="#7c3aed"/>
    <rect x="304" y="344" width="64" height="48" fill="#7c3aed"/>
    <rect x="176" y="392" width="48" height="32" fill="#38bdf8"/>
    <rect x="288" y="392" width="48" height="32" fill="#38bdf8"/>
    <rect x="192" y="184" width="32" height="32" fill="#111827"/>
    <rect x="288" y="184" width="32" height="32" fill="#111827"/>
    <rect x="224" y="232" width="64" height="16" fill="#ec4899"/>
    <rect x="112" y="248" width="48" height="96" fill="#4c1d95"/>
    <rect x="352" y="248" width="48" height="96" fill="#4c1d95"/>
    <rect x="72" y="408" width="368" height="24" fill="#38bdf8"/>
    <rect x="88" y="424" width="336" height="16" fill="#0ea5e9"/>
  </g>
  <text x="256" y="478" text-anchor="middle" font-family="monospace" font-size="34" fill="#f8fafc" font-weight="700">TITAN</text>
</svg>
SVG
...
cat > config.json <<'JSON'
{
  ...
  "workspace": "/Volumes/AI_DRIVE/TitanAgent/workspace",
  "rag_docs": "/Volumes/AI_DRIVE/TitanAgent/rag/docs",
  "rag_db": "/Volumes/AI_DRIVE/TitanAgent/rag/chroma",
  "skills_dir": "/Volumes/AI_DRIVE/TitanAgent/skills",
  "subagents_dir": "/Volumes/AI_DRIVE/TitanAgent/subagents",
  "logs_dir": "/Volumes/AI_DRIVE/TitanAgent/logs",
  "safe_mode": true,
...
mkdir -p skills/app-builder skills/bug-fixer skills/rag-researcher skills/skill-maker

cat > skills/app-builder/SKILL.md <<'MD'
# app-builder

Use when building, scaffolding, upgrading, or refactoring local apps.

Workflow:
1. Inspect workspace.
2. Create a small working scaffold.
3. Add clear files and README.
4. Run the app or test command.
5. Fix errors before final answer.
MD

cat > skills/bug-fixer/SKILL.md <<'MD'
# bug-fixer

Use when there is a traceback, broken app, failed command, or bug.
...
cat > skills/rag-researcher/SKILL.md <<'MD'
# rag-researcher

Use when indexing, searching, or answering from local documents.

Workflow:
1. Inspect rag/docs.
2. Index supported files.
3. Search vector memory.
4. Answer using matching document snippets.
5. Mention source filenames.
MD

cat > skills/skill-maker/SKILL.md <<'MD'
# skill-maker

Use when creating a new local Titan skill.
...
Rules:
- Skill names use lowercase words with hyphens.
- SKILL.md must describe when to use the skill and the workflow.
- Keep skills small and reusable.
MD

cat > subagents/planner.json <<'JSON'
{
  "name": "planner",
  "role": "Break the task into small steps. Prefer practical execution over long theory."
}
JSON

cat > subagents/coder.json <<'JSON'
{
  "name": "coder",
  "role": "Write clean runnable code. Use simple structure. Create complete files."
}
JSON

cat > subagents/tester.json <<'JSON'
{
  "name": "tester",
  "role": "Run safe checks, inspect errors, and verify that the project actually works."
}
JSON

cat > subagents/reviewer.json <<'JSON'
{
  "name": "reviewer",
  "role": "Review for missing files, bugs, unsafe commands, unclear instructions, and weak architecture."
}
JSON
...
def run_command(command):
    ...
    r = subprocess.run(command, shell=True, cwd=str(WORKSPACE), capture_output=True, text=True, timeout=CMD_TIMEOUT)
