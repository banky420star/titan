...
mkdir -p backups docs logs jobs/running jobs/done jobs/cancelled jobs/logs jobs/traces

STAMP="$(date +%Y%m%d_%H%M%S)"
mkdir -p "backups/phase10_$STAMP"

cp control_panel.py "backups/phase10_$STAMP/control_panel.py"

cat > patch_live_trace_viewer.py <<'PY'
...
python3 patch_live_trace_viewer.py
python3 -m py_compile control_panel.py

cat > docs/PHASE10_LIVE_TRACE_VIEWER.md <<EOF
# Phase 10 Live Trace Viewer

Timestamp: $STAMP

Added dashboard job cockpit:
- job list
...
- log tab
- auto-refresh
- chat jobs point users to Jobs tab

Files changed:
- control_panel.py

Backup:
- backups/phase10_$STAMP/control_panel.py

Next:
- workspace file browser
- product browser
- trace streaming improvements
EOF
