...
mkdir -p "backups/phase15_$STAMP"

cp control_panel.py "backups/phase15_$STAMP/control_panel.py" 2>/dev/null || true

cat > patch_toast_notifications.py <<'PY'
...
python3 patch_toast_notifications.py
python3 -m py_compile control_panel.py

cat > docs/PHASE15_TOAST_NOTIFICATIONS.md <<EOF
# Phase 15 Toast Notifications
...
- Automatic toast wrapping for dashboard POST actions
- Startup dashboard-ready toast
- Command palette action toast

Backup:
- backups/phase15_$STAMP/control_panel.py
