...
mkdir -p agent_core subagents docs backups logs

STAMP="$(date +%Y%m%d_%H%M%S)"
mkdir -p "backups/phase3_$STAMP"

cp titan_terminal.py "backups/phase3_$STAMP/titan_terminal.py" 2>/dev/null || true
cp config.json "backups/phase3_$STAMP/config.json" 2>/dev/null || true

echo "[1/4] Writing subagent configs..."

cat > subagents/planner.json <<'JSON'
{
  "role": "planner",
  "name": "Planner Voss",
  "codename": "planner_voss",
  "description": "Planning, decomposition, execution strategy, and project structure.",
  "style": "clear, tactical, concise"
}
JSON
...
cat > subagents/coder.json <<'JSON'
{
  ...
  "name": "Milton",
  "codename": "milton",
  "description": "Coding, implementation, file changes, scaffolding, and practical build steps.",
  "style": "direct, implementation-focused, minimal fluff"
}
JSON
...
cat > subagents/tester.json <<'JSON'
{
  ...
  "name": "Tripwire",
  "codename": "tripwire",
  "description": "Testing, verification, failure checks, edge cases, and smoke tests.",
  "style": "skeptical, precise, verification-first"
}
JSON
...
cat > subagents/reviewer.json <<'JSON'
{
  ...
  "name": "Blackglass",
  "codename": "blackglass",
  "description": "Review, risk, quality, safety, architecture critique, and final polish.",
  "style": "sharp, cautious, quality-focused"
}
JSON

echo "[2/4] Writing agent_core/subagents.py..."

cat > agent_core/subagents.py <<'PY'
...
echo "[3/4] Patching titan_terminal.py with /agents and /team..."
...
if "def show_agents(" not in text:
    marker = "def repl():"
...
def show_agents():
    try:
        ...
        say_panel("Could not load subagents: " + repr(e), title="Agents", style="red")
...
def run_team_task(task):
    try:
        ...
        say_panel(result, title="Titan Team", style="magenta")
    except Exception as e:
        say_panel("Team failed safely: " + repr(e), title="Titan Team", style="red")


'''
    text = text.replace(marker, helpers + marker)

if 'lower == "/agents"' not in text:
...
'''
    if target not in text:
        # fallback: insert before /dashboard handler
        target = '''            if lower in ["/dashboard", "launch dashboard", "open dashboard", "start dashboard"]:
                launch_dashboard()
                continue
'''
...
''' + target
        if target not in text:
            raise SystemExit("Could not find insertion point for /agents and /team.")

    text = text.replace(target, replacement, 1)

# Add to help text if simple string exists.
text = text.replace(
    "/models      Show model config\n",
    "/models      Show model config\n/agents      Show Titan subagents\n/team <task>  Run Planner Voss, Milton, Tripwire, and Blackglass\n"
)

path.write_text(text)
print("Patched terminal with /agents and /team.")
PY

echo "[4/4] Verifying..."

python3 -m py_compile agent_core/subagents.py titan_terminal.py

cat > docs/PHASE3_SUBAGENTS.md <<EOF
# Phase 3 Subagents
