...
mkdir -p "backups/phase14_$STAMP"

cp control_panel.py "backups/phase14_$STAMP/control_panel.py" 2>/dev/null || true

cat > patch_command_palette.py <<'PY'
...
python3 patch_command_palette.py
python3 -m py_compile control_panel.py

cat > docs/PHASE14_COMMAND_PALETTE.md <<EOF
# Phase 14 Command Palette

Timestamp: $STAMP

Added:
- Cmd+K / Ctrl+K dashboard command palette
...
Backup:
- backups/phase14_$STAMP/control_panel.py
